...
## 说明
该 repo 搜集了部分面试题，欢迎大家踊跃参加，给出思路和完整的代码（附单元测试)。

## git usage

下面的链接是git的使用指南，如果不熟悉可以作为参考。

[An Introduction to GitHub](http://xudaoli.dn.qbox.me/talks/teamwork-with-git-flow/)


### 参与方式
本`repo`中包含一个面试题的`doc`文档，里面题目有已经编了号。为每个问题单独建立一个文件夹，文件夹名称就是题目的编号。比如编号为1的问题，建立一个文件夹名称为 `001`。我们规定只能用标准的 C
语言来解题。其他语言一概拒绝。

### 示例：  
000 文件夹  
>   |- `demo.c`   问题0的`ansic c`语言实现版本   
...
[5.给定一个存放整数的数组，重新排列数组使得数组左边为奇数，右边为偶数。  ](https://github.com/wangjinlei/interview-problems/tree/develop/005)
...
14.题目：数组中有一个数字出现的次数超过了数组长度的一半，找出这个数字。    
...
主要是考察递归的算法，迭代算法比较直观。相对来说递归法较难写。


148  
已知一个m行n列的矩阵，其中每行元素按照递增顺序排列，每列元素也是按照递增有序。现在给定一个数，要求查找出该数字在矩阵中的位置。

func lookup(matrix[m][n], int value) (row int, col int)
